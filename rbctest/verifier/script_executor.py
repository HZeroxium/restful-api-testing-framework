    ...
    for index, row in df.iterrows():
        ...
        for i, response_body in enumerate(response_bodies):
            ...
            file_path = os.path.abspath(
                os.path.join(dataset_folder, operation, "responseBody", f"{i}.json")  # type: ignore
            ).replace("\\", "/")
            os.makedirs(
                os.path.abspath(
                    os.path.join(dataset_folder, operation, "responseBody")  # type: ignore
                ),
                exist_ok=True,
            )
        ...
        if is_req_res:
            ...
            for i, request_info in enumerate(request_information):
                ...
                file_path = os.path.abspath(
                    os.path.join(
                        dataset_folder, operation, "queryParameters", f"{i}.json"  # type: ignore
                    )  # type: ignore
                ).replace("\\", "/")
                os.makedirs(
                    os.path.abspath(
                        os.path.join(dataset_folder, operation, "queryParameters")  # type: ignore
                    ),
                    exist_ok=True,
                )
                with open(file_path, "w") as f:
            ...
            for i, request in enumerate(request_body):
                ...
                file_path = os.path.abspath(
                    os.path.join(
                        dataset_folder, operation, "bodyParameters", f"{i}.json"  # type: ignore
                    )  # type: ignore
                ).replace("\\", "/")
                os.makedirs(
                    os.path.abspath(
                        os.path.join(dataset_folder, operation, "bodyParameters")  # type: ignore
                    ),
                    exist_ok=True,
                )
                with open(file_path, "w") as f:
        ...
        with open("code/{index}.py".format(index=index), "w") as f:
            f.write(executable_script)  # type: ignore
