...
if __name__ == "__main__":
    ...
    # )
    # sequence_runner.run_all()
    test_case = ["_projects_id_repository_branches_branch__DELETE_1_1"]
    # endpoint = None
    # endpoint = ["_api_v1_Publications_publicationId_Documents_documentId_Download_GET_2_1"]
    bill_url = "https://bills-api.parliament.uk/"
    gitlab_url = "http://localhost:30000/api/v4"
    canada_url = "https://canada-holidays.ca/api"
    pet_store_url = "https://petstore3.swagger.io/api/v3"
    sequence_runner = SequenceRunner(
        service_name="Pet Store",
        skip_preload=False,
        base_url=pet_store_url,
        out_file_name=f"{datetime.now().strftime('%Y%m%d%H%M%S')}",
        headers=headers,
        sampling_strategy="random_quota",
        want_2xx=20,
